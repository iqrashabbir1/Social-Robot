...
The actual CS3 benchmark implementation now contains more explicit model detail than the earlier high-level paper framing. `B0` is the preserved DeepFace-style visual baseline already stored in the repository log. `B1` is a configurable classical benchmark family over concatenated handcrafted multimodal features and currently supports SVM, Random Forest, Logistic Regression, Extra Trees, and optional boosting-style baselines. `B2` is a deep late-fusion MLP family implemented with `MLPClassifier`, trained one epoch at a time with explicit progress logging. `B3` is a lightweight transformer-style fusion surrogate that projects video, audio, and context into a shared hidden space, computes attention-like fusion weights, and trains an online logistic classifier over the fused representation. The comparison pipeline also supports `H1`, a hybrid majority-vote ensemble across selected classical, deep, and transformer members. A dedicated technical description is provided in `docs/paper1/model_descriptions.md`.
